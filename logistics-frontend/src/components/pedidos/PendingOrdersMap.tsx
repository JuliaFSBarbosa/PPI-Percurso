...
export function PendingOrdersMap({ pedidos }: Props) {
  const mapRef = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    if (!mapRef.current) return;

    const validPoints = pedidos
      .map((p) => {
    ...
    if (validPoints.length === 0) {
      mapRef.current.innerHTML = "Nenhum pedido pendente com coordenadas para mostrar no mapa.";
      return;
    }
    ...
    const init = async () => {
      ...
      map = L.map(mapRef.current).setView([validPoints[0].latitude, validPoints[0].longitude], 12);
  ...
  return (
    <section className={`${styles.card} ${styles.map}`} style={{ marginBottom: 16 }}>
      <div className={styles["card-head"]}>
        <h3>Pedidos pendentes no mapa</h3>
      </div>
      <div
        ref={mapRef}
        style={{
