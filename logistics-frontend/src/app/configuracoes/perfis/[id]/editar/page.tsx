...
export default function EditarPerfilPage() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      setError(null);
      try {
        const [screensResp, profileResp] = await Promise.all([
          fetchProfileScreens(),
          fetchProfileById(params.id),
        ]);
        if (!active) return;
        setScreens(screensResp);
        setForm({
          name: profileResp.name,
          permissions: (profileResp.permissions as ScreenId[]) || [],
          feature_permissions: (profileResp.feature_permissions as Partial<Record<ScreenId, string[]>>) || {},
        });
      } catch (err) {
        if (!active) return;
        ...
        <section className={styles.card}>
          ...
          <form className={styles.form} onSubmit={handleSubmit}>
            ...
            <div className={styles.field}>
              ...
              {loading && <p className={styles.muted}>Carregando dados...</p>}
              {!loading && screens.length === 0 && (
                <p className={styles.muted}>Nenhuma tela disponível para seleção.</p>
              )}
              {!loading && screens.length > 0 && (
